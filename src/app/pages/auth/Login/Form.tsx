...
function Form() {
    ...
    useEffect(() => {
        const isRemembered = localStorage.getItem("rememberedAccount");
        if (isRemembered) {
            const getRememberedData = JSON.parse(isRemembered)
            setEmail(getRememberedData?.email);
            setPassword(getRememberedData?.password);
        }
    }, [])
