...
export async function sendMessageToBot(
    ...
    try {
        ...
        // ✅ FIX: Check HTTP status instead of reading the body early.
        // response.clone().text() was waiting for the ENTIRE response before
        // resolving — making streaming physically impossible.
        if (response.status === 429) {
            ...
            setSelectedConvo(prev => {
                ...
                const updatedConvo: Convo = {
                    ...
                    messagesUi: [
                        ...prev.messagesUi,
                        { role: "model", message: `Error Occurred: ${data.message ?? "Quota Limit Reached!"}` }
                    ]
                };
        ...
        while (true) {
            ...
            // Forces React to flush this update immediately instead of batching,
            // which is what produces the visible per-chunk streaming effect.
            flushSync(() => {
                setSelectedConvo(prev => {
                    if (!prev) return undefined;
        ...
        // Functional updater — guaranteed to apply on top of the last
