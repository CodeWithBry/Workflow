...
function ChatBox() {
  const { darkMode, convoLists, userInfo, chatLists, selectedConvo, setSelectedConvo, pauseEffect, setPauseEffect, setChatLists } = useContext(context) as Context;
  ...
  useEffect(() => {
    if (pauseEffect) {
      ...
      setChatLists(prev => prev.map((chat) => {
        return ({ ...chat, convoLists: chat.convoLists.map((convo) => ({ ...convo, isOpen: false })) })
      }))
      return;
    };
