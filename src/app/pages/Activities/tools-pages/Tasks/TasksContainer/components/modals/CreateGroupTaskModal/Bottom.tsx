...
export default function Bottom(props: GTMBottomProps & GTMProps): JSX.Element {
    ...
    const { taskClass, setSelectedTaskClass, selectedTaskClass, setAllowChanges, userInfo, setWeekData, setAnalytics } = useContext(context) as Context;
    ...
    return (
        <div className={s.bottom}>
            ...
            <Button
                ...
                clickListener={async () => {
                    if (showTaskForm) {
                        const project: TaskClass | undefined = addGroupTask({ pseudoGroup, setPseudoGroup, pseudoTasks, groupName, setSelectedTaskClass, taskClass, taskClassId: selectedTaskClass?.id, setAllowChanges, userId: userInfo?.userId });
                        setShowTaskForm(false);
                        setPseudoGroup(null);
                        setPseudoTasks(null);
                        setGroupName("")
                        setShowGTM(false);
                        if (project && project.dateUpdated && userInfo) {
                            await updateAnalyticsData({
                                updatedGroup: project.taskGroups,
                                project,
