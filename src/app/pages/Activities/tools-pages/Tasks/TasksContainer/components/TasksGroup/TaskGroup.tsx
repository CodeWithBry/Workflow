...
const TaskGroup = memo(function TaskGroup(props: TaskGroupProps) {
  // CONTEXT & PROPS
  const { darkMode, setSelectedTaskClass, setAllowChanges, userInfo, selectedTaskClass, setWeekData } = useContext(context) as Context;
  ...
  const actionLists: ActionsLists[] = useMemo(() => [
    ...
    },
    {
      ...
      functionCall: () => {
        props.setPropsForDTGB(prev => ({...prev, group}))
        props.setShowDTGB(true);
      }
    },
    {
      action: "Pending Selected",
    ...
    },
    {
      ...
      functionCall: () => {
        ...
        group.tasks.forEach((task) => updateTask({
          ...
          task, groupId: group.groupId, userId: userInfo?.userId,
          isMultipleTarget: true, setWeekData
        }))
      }
    },
