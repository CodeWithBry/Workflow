...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, type User, signOut, FacebookAuthProvider, initializeAuth, browserPopupRedirectResolver, browserLocalPersistence, indexedDBLocalPersistence } from "firebase/auth";
...
// convoData: {data...}
// Users > :userId > Analytics > :analyticsId > {analyticsData}, weekList > :date > data() 
...
export async function saveUserData(userId: string, userInfo: UserInfo) {
...
export async function updateProjectLists(userId: string, projectLists: TaskClassLists[]) {
    const userRef = doc(firestore, "Users", userId);
    try {
        await updateDoc(userRef, {
            projectLists
...
export async function getAnalyticsData(userId: string, analyticsId: string): Promise<Analytics> {
    const docRef = doc(firestore, "Users", userId, "Analytics", analyticsId);
    try {
        const requestData = await getDoc(docRef);
        const getData = (requestData.data())?.analytics as Analytics;
        return getData;
    } catch (error) {
        const err = isFirebaseError(error);
        throw new Error(err);
...
export async function setAnalyticsData(userId: string, analyticsId: string, analytics: Analytics) {
    const docRef = doc(firestore, "Users", userId, "Analytics", analyticsId);
    try {
        await setDoc(docRef, {
            analytics
...
export async function updateAnalytics(userId: string, analytics: Analytics) {
    const userRef = doc(firestore, "Users", userId, "Analytics", analytics.id);
    try {
        await updateDoc(userRef, {
            analytics
        })
...
export async function saveWeeksData(userId: string, analyticsId: string, weekData: AnalyticsWeekData) {
    const docRef = doc(firestore, "Users", userId, "Analytics", analyticsId, "Weeks", weekData.date);
    try {
        await setDoc(docRef, {
            weekData
        })
...
export async function getWeekData(userId: string, analyticsId: string, date: string): Promise<AnalyticsWeekData> {
    const docRef = doc(firestore, "Users", userId, "Analytics", analyticsId, "Weeks", date);
    try {
        const requestData = await getDoc(docRef);
        const getData = (requestData.data())?.weekData as AnalyticsWeekData;
        return getData;
    } catch (error) {
        const err = isFirebaseError(error);
        throw new Error(err);
...
export async function deleteConvoData(userId: string, chatId: string, convoId: string) {
    const convoRef = doc(firestore, "Users", userId, "Chats", chatId, "Convos", convoId);
    try {
...
export async function getBookMarkedMessages(userId: string, chatId: string): Promise<MessagesUi[]> {
    const bookMarkRef = doc(firestore, "Users", userId, "Chats", chatId);
    try {
        const requestData = (await getDoc(bookMarkRef))?.data()?.messages as MessagesUi[];
        return requestData
    } catch (error) {
        const err = isFirebaseError(error);
        throw new Error(err);
...
export async function saveBookMarkedMessages(userId: string, chatId: string, messages: MessagesUi[]) {
    const bookMarkRef = doc(firestore, "Users", userId, "Chats", chatId);
    try {
        await setDoc(bookMarkRef, { messages });
    } catch (error) {
        const err = isFirebaseError(error);
        throw new Error(err);
    }
